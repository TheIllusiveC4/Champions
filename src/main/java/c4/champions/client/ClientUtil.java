...
public class ClientUtil {
      ...
      if (client.world != null) {
        ...
        List<Entity> list = client.world.getEntitiesInAABBexcluding(entity,
            entity.getEntityBoundingBox()
                ...
                .grow(1.0D, 1.0D, 1.0D), Predicates.and(EntitySelectors.NOT_SPECTATING,
        ...
        for (int j = 0; j < list.size(); ++j) {
          ...
          if (axisalignedbb.contains(vec3d)) {
            ...
            if (d3 < d2 || d2 == 0.0D) {
              if (entity1.getLowestRidingEntity() == entity.getLowestRidingEntity() && !entity1
                  .canRiderInteract()) {
                if (d2 == 0.0D) {
                  pointedEntity = entity1;
        ...
        if (pointedEntity != null && vec3d.distanceTo(vec3d3) > distance) {
          pointedEntity = null;
          objectMouseOver = new RayTraceResult(RayTraceResult.Type.MISS, vec3d3, EnumFacing.UP,
              new BlockPos(vec3d3));
        }
  ...
  public static void renderChampionHealth(EntityLiving living, IChampionship chp) {
    ...
    if (num <= 18) {
      ...
      Gui.drawModalRectWithCustomSizedTexture(
          startX - client.fontRenderer.getStringWidth(count) / 2, yOffset + 1, 0, 0, 9, 9, 9, 9);
      client.fontRenderer.drawStringWithShadow(count,
          startX + 10 - client.fontRenderer.getStringWidth(count) / 2.0f, 2, 16777215);

    }

    if (renderName) {
      String s =
          living.hasCustomName() ? living.getDisplayName().getFormattedText() : chp.getName();
      client.fontRenderer.drawStringWithShadow(s,
          xOffset + (float) (i / 2 - client.fontRenderer.getStringWidth(s) / 2),
  ...
  @Nullable
  public static RayTraceResult rayTraceBlocks(World world, Vec3d vec31, Vec3d vec32,
    ...
    if (!Double.isNaN(vec31.x) && !Double.isNaN(vec31.y) && !Double.isNaN(vec31.z)) {
      ...
      if (!Double.isNaN(vec32.x) && !Double.isNaN(vec32.y) && !Double.isNaN(vec32.z)) {
        ...
        if ((!ignoreBlockWithoutBoundingBox
            ...
            && block.getRenderLayer() == BlockRenderLayer.SOLID && block
        ...
        while (k1-- >= 0) {
          ...
          if (!ignoreBlockWithoutBoundingBox || iblockstate1.getMaterial() == Material.PORTAL
            ...
            if (block1.getRenderLayer() == BlockRenderLayer.SOLID && block1
                .canCollideCheck(iblockstate1, stopOnLiquid)) {
              ...
              raytraceresult2 = new RayTraceResult(RayTraceResult.Type.MISS, vec31, enumfacing,
                  blockpos);
            }
          }
        }
