...
public abstract class AbstractBulletEntity extends Projectile
{
    ...
    private void selectNextMoveDirection(@Nullable Direction.Axis axis)
    {
        ...
        if (!blockpos.closerThan(this.blockPosition(), 2.0D))
        {
            ...
            if (axis != Direction.Axis.X)
            {
                ...
                if (blockpos1.getX() < blockpos.getX() && this.level.getBlockState(blockpos1.east()).getBlock() instanceof AirBlock)
                {
                    list.add(Direction.EAST);
                }
                else if (blockpos1.getX() > blockpos.getX() && this.level.getBlockState(blockpos1.west()).getBlock() instanceof AirBlock)
                {
            ...
            if (axis != Direction.Axis.Y)
            {
                ...
                if (blockpos1.getY() < blockpos.getY() && this.level.getBlockState(blockpos1.above()).getBlock() instanceof AirBlock)
                {
                    list.add(Direction.UP);
                }
                else if (blockpos1.getY() > blockpos.getY() && this.level.getBlockState(blockpos1.below()).getBlock() instanceof AirBlock)
                {
            ...
            if (axis != Direction.Axis.Z)
            {
                ...
                if (blockpos1.getZ() < blockpos.getZ() && this.level.getBlockState(blockpos1.south()).getBlock() instanceof AirBlock)
                {
                    list.add(Direction.SOUTH);
                }
                else if (blockpos1.getZ() > blockpos.getZ() && this.level.getBlockState(blockpos1.north()).getBlock() instanceof AirBlock)
                {
            ...
            if (list.isEmpty())
            {
                ...
                for (int i = 5; !(this.level.getBlockState(blockpos1.relative(direction)).getBlock() instanceof AirBlock) && i > 0; --i)
                {
                    direction = Direction.getRandom(this.random);
                }
            }
    ...
    public void tick()
    {
        ...
        if (this.level.isClientSide)
        {
            this.level.addParticle(this.getParticle(), this.position().x - vector3d1.x,
              this.position().y - vector3d1.y + 0.15D, this.position().z - vector3d1.z, 0.0D, 0.0D, 0.0D);
        }
    ...
    protected void bulletHit(HitResult result)
    {
        ...
        else
        {
            ((ServerLevel) this.level)
              .sendParticles(ParticleTypes.EXPLOSION, this.position().x, this.position().y, this.position().z, 2,
                0.2D, 0.2D, 0.2D, 0.0D);
    ...
    public boolean hurt(@Nonnull DamageSource source, float amount)
    {
        if (!this.level.isClientSide)
        {
            ...
            ((ServerLevel) this.level)
              .sendParticles(ParticleTypes.CRIT, this.position().x, this.position().y, this.position().z, 15,
                0.2D, 0.2D, 0.2D, 0.0D);
            this.remove(RemovalReason.DISCARDED);
        }
        return true;
