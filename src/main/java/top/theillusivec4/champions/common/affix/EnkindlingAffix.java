...
public class EnkindlingAffix extends GoalAffix {
    ...
    public List<Tuple<Integer, Goal>> getGoals(IChampion champion) {
        return Collections
                .singletonList(new Tuple<>(0, new AttackGoal((Mob) champion.getLivingEntity())));
    }
                    ...
                    if (sqrDistance < 400.0D) {
                        if (this.attackTime <= 0) {
                            ...
                            this.mobEntity.level.addFreshEntity(
                                    new EnkindlingBulletEntity(this.mobEntity.getLevel(), this.mobEntity, livingentity, this.mobEntity.getMotionDirection().getAxis()));
